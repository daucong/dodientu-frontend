    ...
    <v-col lg="10" class="px-6 rounded bg-background">
        ...
        <div class="container">
            <div class="row mb-3">
                <div class="col-12">
                    <div class="row">
                        <div class="col-5"><h2 class="listNews">{{ $t('sidebar.admin.categories.ward') }}</h2></div>
        ...
        <v-table fixed-header lg="12" id="table" class="br-none bg-white">
            ...
            <tbody>
            <tr v-for="entry in wards" :key="entry.id" class="br-none">
                <td class="row-tb text-tb-content"> {{entry.id}}</td>
                <td class="row-tb text-tb-content"> {{entry.name}}</td>
                <td class="row-tb text-tb-content"> {{entry.district.name}}</td>
                <td class="row-tb text-tb-content"> {{entry.district.province.name}}</td>
                <td>
            ...
            <v-card>
                <v-card-text>
                    <v-container>
                        <v-row>
                            ...
                            <v-col cols="9">
                                <v-text-field
                                        ...
                                        v-model="ward.name"></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row>
                            ...
                            <v-col cols="9">
                                <v-select
                                        ...
                                        @update:modelValue="getDistrictsByProvinceId"
                        ...
                        <v-row>
                            <v-col cols="3" class="align-self-center"><span class="text-label-form">{{ $t('form.label-district') }}</span>
                            </v-col>
                            <v-col cols="9">
                                <v-select
                                        v-model="ward.district.id"
                                        :placeholder="$t('form.place-select')"
                                        :items="districts"
                                        item-title="name"
                                        item-value="id"
    ...
    export default {
        ...
        methods: {
            ...
            getCategory() {
                ...
                CategoriesService.list("ward", params)
                    .then(response => {
            ...
            findByID(id) {
                var params = {};
                params["isDelete"] = 'false';
                CategoriesService.findByID("ward", params, id)
                    .then(response => {
                        this.ward = response.data;
                        this.provinceInput = this.ward.district.province.id
                    })
                    .catch(e => {
                        this.notification(e.response.data.message, "error")
                    });
            ...
            save() {
                CategoriesService.add("ward", this.ward)
                    .then(() => {
                        this.getCategory();
                        this.resetModel();
                        this.dialog = false;
            ...
            deletebyId(id) {
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            CategoriesService.delete("ward", id).then(response => {
                                this.getCategory();
