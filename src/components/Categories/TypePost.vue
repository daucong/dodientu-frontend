    ...
    <v-col lg="10" class="px-5 rounded bg-background">
        ...
        <div class="container">
            <div class="row mb-3">
                <div class="col-12">
                    <div class="row">
                        <div class="col-5"><h2 class="listNews">{{ $t('sidebar.admin.categories.type-post') }}</h2></div>
            ...
            <div class="row">
        <v-table fixed-header lg="12" id="table" class="br-none">
            ...
            <tbody>
            <tr v-for="entry in typePosts" :key="entry.id" class="br-none ">

                <td class="row-tb text-tb-content">{{entry.id}}</td>
                <td class="row-tb text-tb-content">{{entry.typePostName}}</td>
                <td class="row-tb text-tb-content">{{formatCurrency(entry.typePostPrice)}}</td>
                <td>
                        ...
                        <v-row>
                            ...
                            <v-col cols="9">
                                <v-text-field
                                        ...
                                        v-model="typePost.typePostName"></v-text-field>
                            </v-col>
                            <v-col cols="3" class="align-self-center"><span class="text-label-form">Giá Tiền</span>
                            </v-col>
                            <v-col cols="9">
                                <v-text-field
                                        @type="number"
                                        variant="outlined"
                                        :placeholder="$t('form.place-text')"
                                        v-model="typePost.typePostPrice" type="number"></v-text-field>
                            </v-col>
                        </v-row>
                    </v-container>
                </v-card-text>
    ...
    export default {
        data() {
            return {
                ...
                items: [
                    ...
                    {
                        title: this.$t('sidebar.admin.categories.type-post'),
                        disabled: true,
        ...
        methods: {
            ...
            findByID(id) {
                var params = {};
                params["isDelete"] = 'false';
                CategoriesService.findByID("typepost", params, id)
                    .then(response => {
                        this.typePost = response.data;
                        this.typePost.typePostPrice =this.typePost.typePostPrice.toFixed(3)
                    })
                    .catch(e => {
                        this.notification(e.response.data.message, "error");
                    });
            ...
            save() {
                CategoriesService.add("typepost", this.typePost)
                    .then(() => {
                        this.getCategory();
                        this.resetModel();
                        this.dialog = false;
            ...
            deletebyId(id) {
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            CategoriesService.delete("typepost", id).then(response => {
                                this.getCategory();
            ...
            resetModel() {
                this.typePost.id = null;
                this.typePost.activateDay = null;
                this.typePost.typePostName = null;
                this.typePost.typePostPrice = null;
            }
        },
        watch: {
            dialog() {
